...
public class CreateReviewCommandHandler : IRequestHandler<CreateReviewCommand, Guid>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var review = _mapper.Map<Domain.Discussion>(request);
        ...
        review.Theme = await GetProduct(request.ProductName, cancellationToken)
                         ?? await CreateProduct(request.ProductName, cancellationToken);
        review.CreationDate = DateTime.UtcNow;
        review.Images = await AddImages(request.Images);

        await _context.Discussions.AddAsync(review, cancellationToken);
        await _context.SaveChangesAsync(cancellationToken);

        return review.Id;
    }

    private async Task<Domain.Theme?> GetProduct(string productName,
        CancellationToken cancellationToken)
    {
    ...
    private async Task<Domain.Theme> CreateProduct(string productName,
        CancellationToken cancellationToken)
    {
